...
class Polling :public Layer {
private:
	typedef Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic> Matrix;
	typedef Eigen::MatrixXi IntMatrix; // typedef Eigen::Matrix<int, -1, -1>

	const int m_channel_rows;
	const int m_channel_clos;
	...
	IntMatrix m_loc;
	Matrix m_z; // 활성화(m_a)하기 전에 출력을 저장
	Matrix m_a;
	Matrix m_din;

public:
	Pooling(const int in_width, const int in_height, const int in_channel, const int pooling_width, const pooling_height) :
		Layer(in_width* in_height* in_channel, (in_width / pooling_width)* (in_height / pooling_height)* in_channel), // Layer 생성을 위한 width*height*channel
	...
	void forward(const Matrix& prev_layer_data) {
		...
		const int channel_end = pre_layer_data.size();
		...
		const Scalar* src = pre_layer_data.data();
